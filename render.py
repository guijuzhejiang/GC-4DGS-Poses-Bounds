...
def render_video(source_path, model_path, loaded_pth, views, gaussians, pipeline, background, fps=30):
    ...
    for idx, pose in enumerate(tqdm(render_poses, desc="Rendering progress")):
        # 1) 构造 world->view 矩阵，确保 dtype 与 device
        world_view_np = getWorld2View2(pose[:3, :3].T, pose[:3, 3], np.array([0.0, 0.0, 0.0]), 1.0)
        world_view = torch.tensor(world_view_np, dtype=torch.float32, device="cuda")
        # 2) 直接赋值（不要随意 transpose，除非你确认 getWorld2View2 的输出需要转置）
        rendered_view.world_view_transform = world_view

        # 3) full_proj = projection_matrix @ world_view  （注意乘法顺序）
        #    使用 matmul（或 bmm with unsqueeze）确保形状匹配
        rendered_view.full_proj_transform = torch.matmul(rendered_view.projection_matrix,
                                                         rendered_view.world_view_transform)

        # 4) 正确计算 camera_center（逆矩阵的前三行第 3 列）
        world_view_inv = torch.inverse(rendered_view.world_view_transform)
        rendered_view.camera_center = world_view_inv[:3, 3].clone()
